...
if ! rpm -q grafana;  then
    ...
    . /etc/myawsenv
    CURDIR=$(dirname $0)
    S=$(aws secretsmanager get-secret-value --secret-id ${PROMETHEUSUSERPASSSECRET} --region ${AWSREGION} --query SecretString --output text)
    ...
    sed -i s/\$GRAFANADATASOURCEUSER/$PROMETHEUSUSERNAME/g /etc/grafana/provisioning/datasources/prometheus.yaml
